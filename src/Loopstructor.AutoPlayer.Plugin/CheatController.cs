...
internal sealed class CheatController : IDisposable
{
    ...
    public void Tick()
    {
        ...
        if (currentSceneHandle == _sceneHandle) return;
    ...
    private CheatExecutionResult SetEnabled(bool enabled)
    {
        if (enabled)
        {
            ...
            if (!SaveIsolationPatch.Verified || !PlatformWriteIsolationPatch.Applied || !GameArtifactIsolationPatch.Applied)
            {
                return CheatExecutionResult.Fail(
                    "作弊模式只允许在存档隔离、平台写入阻断和产物重定向均已验证后启用。",
                    "SAFETY_GATE_NOT_READY");
            }
        }
    ...
    private JObject BuildStateData() => new()
    {
