...
<template>
  <div class="objects-workspace">
    <section class="entity-list mechanical-section">
      <header class="section-heading compact-heading">
        <div class="segmented-control">
          <button :class="{ active: active === 'vehicles' }" @click="active = 'vehicles'; refresh()"><TrainFront :size="16" />战车</button>
          <button :class="{ active: active === 'enemies' }" @click="active = 'enemies'; refresh()"><Bug :size="16" />怪物</button>
        </div>
        <button class="icon-button" aria-label="刷新对象" @click="refresh"><RefreshCw :size="17" /></button>
      </header>
    ...
    <section class="entity-editor mechanical-section" :class="{ mutationLocked: store.writeLocked }">
      ...
      <template v-if="selected">
        ...
        <div class="form-grid two-columns">
          <label>属性<select v-model="attributeId" @change="selectAttribute"><option v-for="attribute in attributes" :key="attribute.id" :value="attribute.id">{{ attribute.name || attribute.id }}</option></select></label>
          <label>目标值<input v-model.number="attributeValue" type="number" step="0.1" /></label>
        </div>
        <div class="action-bar right">
          <button class="button primary" :disabled="store.writeLocked || !attributeId" @click="modify"><Save :size="17" />应用修改</button>
          <button v-if="active === 'vehicles'" class="button danger" :disabled="store.writeLocked" @click="store.command('cheat.removeVehicle', { vehicleId: selected.vehicleId || selected.runtimeId }).then(refresh)"><Trash2 :size="17" />删除战车</button>
        </div>
        ...
        <div v-if="active === 'vehicles'" class="enchantment-editor">
          <label>附魔<select v-model="enchantmentId"><option v-for="item in enchantments" :key="item.id" :value="item.id">{{ item.name || item.enumName || item.id }} · {{ item.enumName || item.id }}</option></select></label>
          <label>层数（0 为删除）<input v-model.number="enchantmentLevel" type="number" min="0" max="2147483647" /></label>
          <button class="button secondary" :disabled="store.writeLocked || !enchantmentId" @click="setEnchantment"><Save :size="17" />设置附魔</button>
        </div>
      </template>
