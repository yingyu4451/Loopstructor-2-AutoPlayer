...
<template>
  <div class="items-workspace grid h-full min-h-0">
    <section class="card inventory-column mechanical-section min-h-0">
      <header class="section-heading compact-heading">
        ...
        <button class="btn btn-error btn-sm button danger compact" :disabled="store.writeLocked" @click="store.command('cheat.clearConsumables').then(() => store.refreshState())"><Trash2 :size="16" />全部删除</button>
      </header>
      <label class="input input-bordered search-box"><Search :size="17" /><input v-model="consumableSearch" name="consumable-search" autocomplete="off" aria-label="搜索消耗品" placeholder="搜索消耗品…" /></label>
    ...
    <section class="card inventory-column mechanical-section min-h-0">
      ...
      <label class="input input-bordered search-box"><Search :size="17" /><input v-model="catapultSearch" name="catapult-search" autocomplete="off" aria-label="搜索弹射点" placeholder="搜索弹射点…" /></label>
      ...
      <label class="field-delete-toggle" :class="{ disabled: store.writeLocked }">
        <input class="checkbox checkbox-success" name="field-catapult-delete-mode" type="checkbox" :checked="fieldDeleteMode" :disabled="store.writeLocked" @change="setFieldDeleteMode(($event.target as HTMLInputElement).checked)" />
        <span class="switch-track"><span /></span>
        <span>点击场上弹射点直接删除</span>
      </label>
      <footer class="inventory-action-footer">
        <button class="btn btn-error btn-sm button danger compact" :disabled="store.writeLocked" @click="store.command('cheat.clearBackpackCatapultPoints').then(() => store.refreshState())">清空背包</button>
        <button class="btn btn-error btn-sm button danger compact" :disabled="store.writeLocked" @click="store.command('cheat.clearFieldCatapultPoints').then(() => store.refreshState())">清空场上</button>
      </footer>
    </section>
  </div>
</template>
