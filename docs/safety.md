...
作弊工具支持获取或删除带多种自选附魔的战车、编辑已有战车附魔（`0` 级移除目标项）、获取消耗品、获取或按实际背包行删除两类弹射点、单删或清空场上弹射点、基地无敌、结束波次、清除敌人、修改车辆或敌人、显示敌人 ID，以及获取、删除或一键补齐遗物。一键补齐只处理游戏 `AllSuperModuleRewards` 中实际配置且尚未持有的遗物，并逐帧执行、逐项验证，避免重复叠加及单帧集中实例化。所有选择器同时显示中文名、枚举名和图标，并可按中文名、枚举名或稳定 ID 搜索；对象战车还显示已有附魔图标。怪物生成点可在捕获状态下用左 Alt 加鼠标左键反复添加，游戏内显示编号十字与坐标，工具中可单删或清空。生成默认复用当前波次 `CurrentAILevel` 及游戏正式难度倍率链，显式自定义等级才覆盖；每点按半径分散，生成后还要通过阵营、碰撞、战斗和可受击校验。玩家模式使用这些能力前应备份存档；任何带作弊标记的结果都不得与干净回归结果混合统计。
