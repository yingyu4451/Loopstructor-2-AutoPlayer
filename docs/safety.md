...
- 默认防线会把子命令结果嵌套在返回结构中。检查器只沿当前写命令的有效结果包查找 `statePolluted = true` 或 `needsReset = true`，并跳过 `before`、`previous`、`old*`、`history` 等历史快照。动力站点步骤曾提交但后续失败时，必须验证最终轨道、车列和已放置战车：已回到干净检查点可以重试；无法确认完整回滚才按 Unsafe 处理、停止动作并要求全新游戏进程。只读查询失败不属于状态污染。
...
- 轨道插点、普通/动力点放置和特殊点移动属于结构写事务。命令一旦发出，即使返回 Pending、查询暂时失败、用户请求暂停或停止，也只能继续只读对账；不得重发写命令或清空事务身份。只有确认交互结束、目标身份与轨道结构符合预期后才解除门禁。
