...
```text
%LOCALAPPDATA%\LoopstructorAutoPlayer\profiles\<qa-profile-id>
```
...
作弊调试是独立的显式授权模式，不是普通自动游玩会话中的开关：

- 必须在 Manager 启动游戏前勾选“作弊调试会话（使用一次性隔离档）”。授权被写入本次环境/启动票据并绑定游戏根目录，游戏启动后不能补授予；
- 每次作弊启动都创建 `profiles\<game-id>\cheat\<session-id>` 一次性存档，不能选择普通 QA profile 或真实玩家存档；
- 安全握手成功后仍需在独立作弊工具窗口中手动启用。没有授权、隔离门禁未通过、作弊协议不匹配或未启用时，写命令全部拒绝；
- 作弊授权会话始终禁止自动游玩，即使尚未启用作弊或只执行了只读查询；要恢复普通自动测试，必须关闭游戏、取消勾选作弊会话并重新启动；
- 在已启用的作弊模式中，任何会改变状态的操作一经尝试，无论成功、失败、超时还是响应结果不确定，都设置 `CheatUsed` 和 `NeedsProcessRestart`，本次运行结果不再可作为干净测试证据；
- 场景切换会关闭基地无敌和敌人 ID 显示等瞬态功能。Manager 连接或心跳租约丢失时，插件会自动关闭作弊模式和全部瞬态功能，不能留下无人控制的持续修改；
...
作弊工具支持获取带可选附魔的指定战车、获取消耗品、基地无敌、结束当前波次、清除当前敌人、修改指定车辆属性、修改指定敌人属性、显示敌人运行时 ID、获得遗物及在指定位置生成允许的怪物。这些能力只用于可丢弃 QA 环境；启用基地无敌或直接修改属性后的玩法结果不得与普通回归结果混合统计。
...
- 确认证据磁盘空间充足。
- 需要作弊调试时，在启动游戏前勾选一次性隔离作弊会话；普通回归保持不勾选。
...
- 检查平台写入阻断状态；
- 普通自动游玩会话再发送 `start`。
- 作弊会话不要发送 `start`；应打开作弊工具并再次手动启用，确认界面显示当前会话已授权。
