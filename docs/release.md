...
```powershell
.\scripts\package.ps1 -Version 0.6.30
```

已经完成同版本 Release 构建时：

```powershell
.\scripts\package.ps1 -Version 0.6.30 -SkipBuild
```
...
```text
artifacts/release/
Loopstructor.AutoPlayer-0.6.30-win-x64.zip
Loopstructor.AutoPlayer-0.6.30-win-x64.zip.sha256
Loopstructor.AutoPlayer-0.6.29-to-0.6.30-win-x64.delta.zip        可选
Loopstructor.AutoPlayer-0.6.29-to-0.6.30-win-x64.delta.zip.sha256 可选
  autoplayer-update-manifest.json
```

完整 Release ZIP `Loopstructor.AutoPlayer-0.6.30-win-x64.zip` 始终用于手动下载、首次安装、跨版本升级和增量不可用时的回退。必须先完整解压，不能直接在资源管理器的 ZIP 预览中运行。压缩包内只有固定的 `Loopstructor 2.AutoPlayer\` 顶层目录，目录名不包含版本号；进入该目录后才是程序根目录：
...
```json
{
  ...
  "assetName": "Loopstructor.AutoPlayer-0.6.30-win-x64.zip",
  "sha256": "<64-lowercase-hex>",
  "size": 64969988,
  "deltaAssets": [
    {
      "fromVersion": "0.6.29",
      "assetName": "Loopstructor.AutoPlayer-0.6.29-to-0.6.30-win-x64.delta.zip",
      "sha256": "<64-lowercase-hex>",
      "size": 2391960
    }
  ]
