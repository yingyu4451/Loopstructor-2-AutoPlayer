...
AutoPlayer 把“桌面呈现”“控制编排”和“游戏内执行”分开：Electron + Vue Desktop 负责统一可见界面，.NET 8 Host 掌握安装、进程连接、玩家存档备份与事务恢复、自动游玩、更新和本机 IPC；BepInEx 插件只接受当前 Windows 用户下、绑定游戏目录和程序集指纹的本机授权。玩家常驻模式允许手动启动游戏后随时连接，隔离 QA 模式继续使用单次授权与独立测试数据。`0.6.55` 的存档页通过白名单 RPC 枚举受管快照并执行确认后的关闭游戏、恢复和重启；更新界面继续复用同一 Electron 运行时的 `--updater` 模式。
...
8. `ResidentPlayer` 明确不安装 QA 存档重定向、平台写入阻断或游戏诊断产物重定向；任一标志意外为 true 时 Host 拒绝握手。玩家原存档和平台行为保持游戏默认语义。插件只在 Unity 主线程低频读取 `SaveManager.GetSaveFolderPath` 与地图进度；Host 在进度变化后等待写盘稳定、复制前后核对文件指纹，并将快照保存到工具数据根的 `save-backups`，不会在游戏线程执行文件复制。
