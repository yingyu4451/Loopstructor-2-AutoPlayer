...
1. 将 `Loopstructor.AutoPlayer-0.6.37-win-x64.zip` 完整解压；压缩包内只有一个固定的 `Loopstructor 2.AutoPlayer\` 顶层目录，不在目录名中附加版本号。不要直接在资源管理器的 ZIP 预览中运行程序。
...
push 和 pull request 会运行构建与测试；推送 `v*` tag 会生成完整的 Windows x64 Release ZIP、对应 SHA-256 和 `autoplayer-update-manifest.json`，随后发布 GitHub Release。找到可验证的上一正式版本时，工作流还会生成“上一版本 → 当前版本”的文件级增量 ZIP。`Loopstructor.AutoPlayer-0.6.37-win-x64.zip` 始终保留用于手动下载、首次安装、跨版本升级和完整包回退，内部只有固定的 `Loopstructor 2.AutoPlayer\` 顶层目录。
...
`v0.6.37` 适配游戏当前的独立战车轨道规则：每辆战车作为单独车列，通过始发站按轨道实时 `driverMaxCount` 投放；默认容量 2 以及遗物、特殊站点提供的容量变化都从运行时读取，不再沿用“一轨一车列”或“向旧车列加车厢”的旧判断。地图节点生成的固定普通中继站也会进入始发站断环重连候选，不再只处理可移动特殊站点。本版本以 `v0.6.36` 作为相邻增量更新基线。

`v0.6.36` 修复游戏零基稳定站点 ID 被当成空身份的问题，并将四向覆盖按连续轨道边界而不是站点顶点数量判定。新游戏开局由 `pointId=0` 参与组成的合法三站闭环不再被误判为断轨；后续联合布局、战中重连和视觉证据也统一保留该站点身份。重复边、断裂、自交、多连通分量和未包围基地的轨道仍会被拒绝。本版本以 `v0.6.35` 作为相邻增量更新基线。
