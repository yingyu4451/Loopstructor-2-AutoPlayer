...
1. 将 `Loopstructor.AutoPlayer-0.6.27-win-x64.zip` 完整解压；压缩包内只有一个固定的 `Loopstructor 2.AutoPlayer\` 顶层目录，不在目录名中附加版本号。不要直接在资源管理器的 ZIP 预览中运行程序。
...
push 和 pull request 会运行构建与测试；推送 `v*` tag 会生成完整的 Windows x64 Release ZIP、对应 SHA-256 和 `autoplayer-update-manifest.json`，随后发布 GitHub Release。找到可验证的上一正式版本时，工作流还会生成“上一版本 → 当前版本”的文件级增量 ZIP。`Loopstructor.AutoPlayer-0.6.27-win-x64.zip` 始终保留用于手动下载、首次安装、跨版本升级和完整包回退，内部只有固定的 `Loopstructor 2.AutoPlayer\` 顶层目录。
...
`v0.6.27` 修复地图刚开始打开就提前选择节点的问题：插件读取游戏原生 `Open` Animator，等待 1.55 秒过渡动画真正结束后才重新读取目标并显示 1 秒绿色边框。所有带边框的自动选择统一保留 1 秒，描边向目标内部缩进 2 像素，避免左右边线被遮罩裁切。本版本以 `v0.6.26` 作为相邻增量更新基线。
