...
Loopstructor 2 AutoPlayer 是一个面向 Windows x64 打包游戏的本地调试工具。`v0.6.60` 修复 Skyspine 机械界面的交互态与层级：hover、selected、focus-visible 和 disabled 不再互相覆盖材质，异形边框连续，工作台标题与页面内容不再遮挡，紧凑窗口导航完整可达。`.NET 8 Host` 继续负责游戏验证、插件安装、可信会话、存档、自动游玩、作弊命令和更新交接。
...
.\scripts\package.ps1 -Version 0.6.60 -SkipBuild
```

若只想一步生成发布包，可以在 bootstrap 后运行：

```powershell
.\scripts\package.ps1 -Version 0.6.60
```
...
1. 将 `Loopstructor.AutoPlayer-0.6.60-win-x64.zip` 完整解压；压缩包内只有一个固定的 `Loopstructor 2.AutoPlayer\` 顶层目录，不在目录名中附加版本号。不要直接在资源管理器的 ZIP 预览中运行程序。
