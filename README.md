...
1. 将 `Loopstructor.AutoPlayer-0.6.30-win-x64.zip` 完整解压；压缩包内只有一个固定的 `Loopstructor 2.AutoPlayer\` 顶层目录，不在目录名中附加版本号。不要直接在资源管理器的 ZIP 预览中运行程序。
...
push 和 pull request 会运行构建与测试；推送 `v*` tag 会生成完整的 Windows x64 Release ZIP、对应 SHA-256 和 `autoplayer-update-manifest.json`，随后发布 GitHub Release。找到可验证的上一正式版本时，工作流还会生成“上一版本 → 当前版本”的文件级增量 ZIP。`Loopstructor.AutoPlayer-0.6.30-win-x64.zip` 始终保留用于手动下载、首次安装、跨版本升级和完整包回退，内部只有固定的 `Loopstructor 2.AutoPlayer\` 顶层目录。
...
`v0.6.30` 重构轨道自动优化：候选先满足基地包围和四向覆盖，再按当前关卡 `MapPosManager` 的真实普通/能量站最小间距分层，最佳间距层内才比较游戏预览回转周期与 N/T；可移动特殊站点从 `queryDisposable` 的运行时行为动态发现，不依赖枚举白名单。战斗期间禁止直接调动车列或战车，支持的游戏构建会严格通过始发站断环、原生缓存车列、移动站点、预览并重新闭环的玩家流程优化，缺少断环接口时自动降级为只移动可移动站点。本版本以 `v0.6.29` 作为相邻增量更新基线。
